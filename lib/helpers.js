...
_before.loginUser = function(credentials, optionalHandler) {
  before(function(done) {
    var test = this;
    var app = this.app;
    var Person = app.models.person;

    if(typeof credentials === 'function') credentials = credentials();

    Person.findById(credentials.id, function(err, result) {
      if(err) {
        console.error('Error in _before.loginUser:', err);
        done(err);
      } else {
        result.constructor.login(credentials, function(err, token) {
          if(err) {
            console.error('Error in _before.loginUser:', err);
            done(err);
          } else {
